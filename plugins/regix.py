...
async def pub_(bot, message):
    ...
    await send(client, user, "<b>Forwarding Process Stared🕵!!...</b>")
    sts.add(time=True)
    ...
    if locked:
        try:
          ...
          async for message in client.iter_messages(
            ...
            return await stop(client, user)
        temp.IS_FRWD_CHAT.remove(sts.TO)
        await send(client, user, "<b>Forwarding Process Completed✅!!...</b>")
...
async def edit(msg, title, status, sts):
   ...
   if status in ["cancelled", "completed"]:
      button.append(
         ...
         InlineKeyboardButton('Updates', url='https://t.me/botuptest')]
         )
   else:
